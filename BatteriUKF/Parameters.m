...
ocv_LUT = [2.99808472821531,3.29015678205606,3.51449858492796,3.66802196422992,3.76050972589663,3.81021773542304,3.83947699888896,3.87029574398353,3.91996150102984,3.99664318400941,4.09499317158672,4.19174938813381];
...
%% Other Parameters
Ts_micro = 0.2;           % Fundamental sample time   [s]
Ts_macro = 60;

%% Filter parameters
x0  = [0.0; AH0; AH];       % Initial State
Px0 = 1e-4*diag(x0);    % Initial covariance
Qx0 = 1e-4*diag(x0);    % Process noise covariance
...
k0 = [0.0, AH0, AH, Rs, Rp, tau];
Pk0 = 1e-4*diag(k0);    % Initial covariance
Qk0 = 1e-4*diag(k0);    % Process noise covariance
Rk0 = 1e-3;             % Measurement noise covariance

l_k             = length(k0);

Rs_index      = C_index + 1;
Rp_index      = Rs_index + LUT_length;
tau_index     = Rp_index + LUT_length;
